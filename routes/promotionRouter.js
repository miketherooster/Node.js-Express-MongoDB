...
.post(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {((req, res, next) => {
...
.put(authenticate.verifyUser, (req, res) => {((req, res) => {
    res.statusCode = 403;
...
.post(authenticate.verifyUser, (req, res, next) => {((req, res) => {
    res.statusCode = 403;
    res.end(`POST operation not supported on /promotions/${req.params.promotionId}`);
})})
.put(authenticate.verifyUser, authenticate.verifyAdmin, (req, res) => {((req, res, next) => {
...
.post(authenticate.verifyUser, (req, res, next) => {((req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
        if (promotion) {
...
.put(authenticate.verifyUser, (req, res) => {((req, res, next) => {
    Promotion.findById(req.params.promotionId)
    .then(promotion => {
        if (promotion && promotion.comments.id(req.params.commentId)) {
            if (req.body.rating) {
